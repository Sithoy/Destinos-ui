...
export function InspirationPage({ slug, onCustomise }: { slug: string; onCustomise: (item: TravelExperience) => void }) {
  ...
  if (item && copy) return <main id="main-content" className="bg-[#fff9f2] text-[#102337]">
    <section className="relative isolate flex min-h-[620px] items-end overflow-hidden bg-slate-800 text-white">
      <SmartImage priority src={item.hero} alt={copy.destination} className="absolute inset-0 -z-20 h-full w-full object-cover" /><div className="absolute inset-0 -z-10 bg-gradient-to-t from-black/85 via-black/30 to-black/10" />
  ...
  return <main id="main-content" className="min-h-[70vh] bg-[#fff9f2] text-[#102337]"><div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
    ...
    <div className="my-10 space-y-4 border-y border-[#e9d9c8] py-6">{[{ values: regions, selected: region, set: setRegion, label: pt ? 'Região' : 'Region' }, { values: styles, selected: style, set: setStyle, label: pt ? 'Estilo de viagem' : 'Travel style' }].map(group => <fieldset key={group.label}><legend className="mb-3 text-xs uppercase tracking-wider text-slate-500">{group.label}</legend><div className="flex flex-wrap gap-2">{['', ...group.values].map(value => <button key={value} type="button" aria-pressed={group.selected === value} onClick={() => group.set(value)} className={`min-h-11 rounded-full border px-4 text-sm ${group.selected === value ? 'border-[#fe8500] bg-[#fe8500] text-[#35180f]' : 'border-[#e0d5c7] bg-white'}`}>{value ? (pt ? value : english[value] || value) : (pt ? 'Todos' : 'All')}</button>)}</div></fieldset>)}</div>
    ...
    {!filtered.length && <div className="py-12"><p>{pt ? 'Ainda não temos uma viagem publicada nesta combinação. Explore todas as ideias ou fale connosco para criar a sua.' : 'No published journeys match these filters yet. Explore all ideas or contact us to create yours.'}</p><button className="mt-4 min-h-11 underline" onClick={() => { setRegion(''); setStyle(''); }}>{pt ? 'Limpar filtros' : 'Clear filters'}</button></div>}
  </div></main>;
}
